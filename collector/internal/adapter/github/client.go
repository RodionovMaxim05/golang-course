...
func (gc *GitHubClient) GetRepo(ctx context.Context, owner, repo string) (domain.Repository, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&info); err != nil {
		gc.log.Error("failed to parse github response", "error", err, "owner", owner, "repo", repo)
		return domain.Repository{}, fmt.Errorf("failed to parse JSON: %w", err)
	}

	gc.log.Debug("repository fetched successfully", "owner", owner, "repo", repo, "stars", info.StargazersCount, "created_at", info.CreatedAt)
